# this is from https://github.com/snap-stanford/pretrain-gnns
...
def mol_to_graph_data_obj_simple(mol):
    ...
    x = torch.LongTensor(np.array(atom_features_list)).type(torch.LongTensor)
    ...
    if len(mol.GetBonds()) > 0: # mol has bonds
        ...
        edge_index = torch.LongTensor(np.array(edges_list).T).type(torch.LongTensor)

        # data.edge_attr: Edge feature matrix with shape [num_edges, num_edge_features]
        #edge_attr = torch.tensor(np.array(edge_features_list),
       #                          dtype=torch.long).to(device)
        edge_attr = torch.LongTensor(np.array(edge_features_list)).type(torch.LongTensor)
